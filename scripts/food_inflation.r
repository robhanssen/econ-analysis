...
inflation <- retrieve_data(index = c("CUSR0000SAF11", "CUSR0000SEFV", "CUSR0000SAF113")) %>%
    pivot_wider(names_from = index, values_from = value) %>%
    drop_na() %>%
    rename(food_at_home = CUSR0000SAF11, food_out = CUSR0000SEFV, food_fruitveg = CUSR0000SAF113) %>%
    mutate(month = month(date), year = year(date)) %>%
    group_by(month) %>%
    mutate(across(2:4, ~ .x / lag(.x) - 1, .names = "{col}_annualinflation")) %>%
    ungroup()

plot_foodinflation <- inflation %>%
    filter(date > ymd(20180101)) %>%
    ggplot() +
    geom_line(aes(x = date, y = food_at_home_annualinflation), color = "blue") +
    geom_line(aes(x = date, y = food_fruitveg_annualinflation), color = "darkgreen") +
    geom_line(aes(x = date, y = food_out_annualinflation), color = "maroon") +
...
plot_foodhome <-
    ...
    labs(
        ...
        caption = "Source: FRED CUSR0000SAF11\nIndexed to the average full-year 2014-2018"
    ) +
    my_theme
...
plot_foodout <-
    ...
    scale_y_continuous(labels = scales::dollar_format(), breaks = 1 + -100:100 * .05)