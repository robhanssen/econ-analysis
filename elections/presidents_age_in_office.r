...
data_cleaned <-
    data_raw %>%
    mutate(across(name_birth_death:term, ~str_remove_all(.x, "\\[\\d{2}\\]"))) %>%
    mutate(across(name_birth_death:term, ~str_remove_all(.x, "\\[\\d{1}\\]"))) %>%
    mutate(across(name_birth_death:term, ~str_remove_all(.x, "\\[[[:lower:]]\\]"))) %>%
    ...
    mutate(across(birth:death, ~ lubridate::floor_date(as.Date(.x, format = "%Y"), unit ="year")))

presidents <-
    data_cleaned %>%
    mutate(age = (death - birth)/lubridate::dyears(1),
           alive = is.na(age),
           age_in_office = (term_start - birth)/lubridate::dyears(1),
           age_out_office = (term_end - birth)/lubridate::dyears(1),
           end_bar = ifelse(is.na(term_end), (lubridate::today()-birth)/dyears(1), age_out_office),
           current_age = ifelse(!is.na(age), NA, (lubridate::today() - birth)/dyears(1)))
...
presidents %>%
    ...
    geom_segment(aes(yend = name, x = age_in_office, xend = end_bar, color = alive), size = 3) + 
    geom_vline(xintercept = mean(presidents$age_in_office), lty = 3) +
    geom_vline(xintercept = mean(presidents$age_out_office, na.rm = TRUE), lty = 3) + 
