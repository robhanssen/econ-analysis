...
ui <- fluidPage(
  ...
  sidebarLayout(
    sidebarPanel(
      # numericInput("cpi", "CPI (CPILFESL)", value = NA, step = 0.01),
      sliderInput("cpi_range", "CPI range", min = 200, max = 400, value = c(last(retrieve_data("CPILFESL")$value), last(retrieve_data("CPILFESL")$value)), step = 0.1),
...
server <- function(input, output, session) {
  ...
  select_data <- reactive({
    ...
    cpi_val <- input$cpi
    if (is.null(cpi_val) || is.na(cpi_val)) cpi_val <- last(mo$dat$CPILFESL)
